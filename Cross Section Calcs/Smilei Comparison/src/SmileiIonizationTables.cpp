...
double SmileiIonizationTables::ionization_energy( int atomic_number, int Zstar )
{
    return ionizationEnergy[( atomic_number*( atomic_number-1 ) )/2 + Zstar];
}

// Gets the azimuthal atomic number of a given ion
int SmileiIonizationTables::azimuthal_atomic_number( int atomic_number, int Zstar )
{
...
double SmileiIonizationTables::binding_energy( int atomic_number, int Zstar, int k )
{
